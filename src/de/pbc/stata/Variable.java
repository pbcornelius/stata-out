...
public class Variable {
	...
	private static final Pattern FLAGS = Pattern
			.compile("(?i)^(?<flags>([cobidn]|(?<val>\\d+)|(?<lag>[lf]\\d*))*)\\.(?<varname>.+)$");
	...
	public Variable(String name, String format) {
		...
		if ((m = FLAGS.matcher(name)).matches()) {
			if (m.group("val") != null)
				value = Double.valueOf(m.group("val"));
			...
			if (m.group("lag") != null) {
				...
				lag = m.group("lag").replaceAll("(?i)[lf]", "").isEmpty() ? 1
...
//		SFIToolkit.displayln(String.format("%1$s o:%2$b b:%3$b d:%4$b", this.name, omitted, base, delta));
	...
	public boolean hasIndex() {
		return getIndex() <= Data.getVarCount() & getIndex() > 0;
	}
	...
	public String getLabel() {
		if (name.equals("_cons")) {
			...
			/*
			 * Sometimes, Stata returns a non-empty label string, even when there is no
			 * label defined in Stata. In this case, one can (1) try and remove the invalid
			 * label in Stata or (2) disable the variable labels here. It looks like this
			 * happened b/c the var name was set to lower case and then the getVarIndex()
			 * didn't return the correct index anymore. Not sure if the flag matching now
			 * still works, but it should since the case-insensitive flag was set in the
			 * pattern.
			 */
